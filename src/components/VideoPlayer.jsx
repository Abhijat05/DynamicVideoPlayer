...
const VideoPlayer = ({ video, videos, setVideos, onSelectVideo }) => {
  ...
  const [player, setPlayer] = useState(null) // Track player instance

  // Modified Plyr options to minimize conflicts
  const plyrOptions = {
    controls: [
      'play-large',
      'play',
    ...
    // Removed settings that might conflict
    settings: ['captions', 'quality', 'speed'],
    speed: { selected: 1, options: [0.5, 0.75, 1, 1.25, 1.5, 2] },
    // Disable keyboard shortcuts to prevent conflicts
    keyboard: { focused: true, global: false },
    tooltips: { controls: false, seek: false },
    captions: { active: false, update: true },
    // Simplify quality options
    quality: {
      default: 720,
      options: [1080, 720, 480, 360]
    },
    // Use a more specific storage key to prevent conflicts
  ...
  const handleVideoError = useCallback((event) => {
    ...
    if (event.detail && event.detail.code) {
      message = errorMessages[event.detail.code] || message
    }
  ...
  useEffect(() => {
    // Keep track of created blob URLs to clean up
    const blobUrls = []
    
    // Add current video's URL to tracking if it's a blob
    ...
    // Cleanup function
    return () => {
      // Revoke any blob URLs we created to prevent memory leaks
  ...
  useEffect(() => {
    // Check if plyr instance is available after component mounts
  ...
  return (
    <div className="bg-card rounded-lg overflow-hidden shadow-lg border border-border transition-all video-player-container">
      ...
      <div className={cn(
        ...
        {/* Video navigation controls - Apply higher z-index */}
        {!isFullscreen && videos.length > 1 && (
          <div className="absolute top-2 right-2 z-20 flex gap-2 p-2 bg-black/50 backdrop-blur-sm rounded-lg">
            ...
            <button
              ...
              disabled={videos.findIndex(v => v.url === video.url) === videos.length - 1}
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* Error overlay - Apply higher z-index */}
        {hasError && (
